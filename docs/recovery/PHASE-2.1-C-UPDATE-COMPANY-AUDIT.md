...
- **`any` restantes no payment:** 5 usos (1 no `query as any`, 1 em `existingLinks[0] as any`, 2 na resolução e criação de LINK as any e 1 no parâmetro `compensationData: any`).
...
## 23. Subfase 2.1-C.2-C-B.1.3 — Correção da frequência na compensação (Finalizado)
- **Model não nullable validado:** Constatou-se no model da entidade (`apps/backend/src/modules/company/models/company.ts`) que `spend_limit_reset_frequency` possui `.default(SpendLimitResetFrequency.NONE)` e **não** é nullable. Deste modo, instâncias íntegras sempre possuem este valor e seu uso dentro dos tipos de resultado (`UpdateCompanyCompensationData` e `UpdateCompanyStepResult`) foi refatorado para ser obrigatório e exato — excluindo union types falhos como `SpendLimitResetFrequency | undefined`.
- **Normalização null para undefined removida:** Os fallbacks perigosos nos builders (que convertiam `null` do banco para `undefined`) foram removidos. Se a database devolver `null` para status ou frequency, a camada de runtime do workflow rejeitará como corrompido, evitando mascarar dados.
- **Snapshot e Resultado exigem frequência:** Conforme modelagem atualizada, ambas as tipagens `UpdateCompanyStepResult` e `UpdateCompanyCompensationData` exigem rigidamente a presença do enum `SpendLimitResetFrequency`. O guard `isUpdateCompanyCompensationData` foi reforçado usando `Object.prototype.hasOwnProperty.call(value, "spend_limit_reset_frequency")`.
- **Rollback unitário preserva NONE:** O teste do `updateCompanyCompensationHandler` agora certifica estritamente que, caso o Snapshot armazene `SpendLimitResetFrequency.NONE`, a property constará devidamente presente (não omitida) e com esse valor exato no payload reconectado pro serviço, mitigando perda de default no banco.
- **Teste sem `as any`:** Um cast de tipo no teste ao instanciar `previous_status: "invalid" as any` foi removido para injetar o parâmetro sob `unknown` e utilizar os Type Guards puros implementados (`isCompanyStatus`), elevando a segurança da suíte base unitária.
- **Caminho correto do teste:** Refletido formalmente como `apps/backend/src/workflows/update-company/__tests__/update-company.unit.spec.ts`.
- **"Payload parcial mantinha propriedades com valor undefined":** A afirmação anterior incorreta tecnicamente de "Object Spread Bug" foi alterada para refletir sua semântica certa onde payload de objetos JS puros destruturados mantêm keys mapeadas, ocasionando substituições nulas se encaminhados direto. Removidas também menções irresponsáveis a corrupção generalizada de ORM sem testes de integração reais para suporte da alegação.
- **Payment any restantes:** As 5 tipagens inseguras `as any` ou explícitas remanescentes operam somente dentro do `createAccountHolderStep` (Payment module/Link), preservadas rigorosamente na mesma forma. Payment não alterado.
- **Pendências Mapeadas:** O rollback completo de integração não testado, assim como a Idempotência continua não comprovada na amplitude de workflow distribuído inteiro (dependente do Payment resolver a sua transação antes).
