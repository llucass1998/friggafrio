...
test.describe("PDP and product shelves", () => {
  for (const viewport of [
    ...
    test(`PDP grid remains aligned at ${viewport.width}px`, async ({ page }) => {
      ...
      await expect(page.getByTestId("product-detail-panels")).toBeVisible()
      ...
      const metrics = await page.evaluate(() => {
        ...
        return {
          ...
          detailWidth: document.querySelector<HTMLElement>("[data-testid=product-detail-panels]")?.getBoundingClientRect().width ?? 0,
        }
      })
  ...
  test("mobile sticky purchase bar parks before the footer and yields to dialogs", async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 })
    await page.goto(`/br/products/${productHandle}`, { waitUntil: "networkidle" })
    const stickyAdd = page.getByRole("button", { name: "Adicionar produto ao carrinho" })
    ...
    await expect(stickyAdd).toHaveCount(0)
  })
  ...
  for (const viewport of [
    ...
    const controls = section.locator("[data-carousel-controls=section-header]")
    await expect(controls).toHaveCount(1)
    const previous = controls.locator("button").nth(0)
    const next = controls.locator("button").nth(1)
