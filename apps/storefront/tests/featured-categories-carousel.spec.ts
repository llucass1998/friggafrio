...
test.describe('Carrossel de Categorias em Destaque (Fase 4-A)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/store/product-categories*', async (route) => {
      await route.fulfill({
        status: 200,
  ...
  test('Deve renderizar os botões de navegação no lado direito do cabeçalho', async ({ page, isMobile }) => {
    // Apenas testar se os botões existem no DOM para ambos, pois as classes hidden mudam visibilidade
    const prevBtn = page.getByRole('button', { name: 'Ver categoria anterior' }).first();
    const nextBtn = page.getByRole('button', { name: 'Ver próxima categoria' }).first();

    // Anexa o dom e confia na verificação da existência.
  ...
  test('Deve renderizar fallback para categorias sem imagem na metadata', async ({ page }) => {
    ...
    await expect(cat2Card.locator('span', { hasText: 'C' })).toBeVisible();
  });
