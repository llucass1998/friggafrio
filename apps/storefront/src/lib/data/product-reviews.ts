...
export const getReviewEligibility = async (productId: string) =>
  sdk.client.fetch<ReviewEligibility>(`/store/products/${productId}/reviews/eligibility`)
...
export const updateProductReview = async (productId: string, reviewId: string, input: { rating: number; title: string; body: string }) =>
  sdk.client.fetch<{ review: ProductReview & { status: "pending" } }>(`/store/products/${productId}/reviews/${reviewId}`, {
    method: "PATCH",
    body: input,
  })
