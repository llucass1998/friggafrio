...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (req.url.includes('/store/regions')) {
      ...
      // Create line item
      res.writeHead(200); // the user said /store/carts returns HTTP 200, but I will log this request anyway
      res.end(JSON.stringify({
        cart: { id: "cart_123", items: [{ id: "item_1", variant_id: "var_123", quantity: 1 }] }
      }));
    }
