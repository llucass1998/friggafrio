...
import { useCreateProductReview, useProductReviews, useReviewEligibility } from "@/lib/hooks/use-product-reviews"
...
function ReviewModal({ productId, productTitle, onClose }: { productId: string; productTitle: string; onClose: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    event.preventDefault()
    if (!rating || body.trim().length < 10 || mutation.isPending) return
    try {
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="review-modal-title" className="max-h-[95vh] w-full overflow-y-auto rounded-t-3xl border border-[var(--color-border)] bg-white shadow-2xl sm:max-w-xl sm:rounded-2xl">
        <div className="border-b border-[var(--color-border)] bg-[var(--color-surface-soft)] px-5 py-5 sm:px-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 id="review-modal-title" className="mt-2 text-xl font-bold text-[var(--color-navy)]">Avaliar produto</h2>
        ...
        {submitted ? (
          ...
          <form onSubmit={submit} className="space-y-5 px-5 py-6 sm:px-7 sm:py-7">
            ...
            {mutation.isError && <p className="rounded-lg bg-rose-50 px-3 py-2.5 text-sm text-rose-700" role="alert">Não foi possível enviar sua avaliação. Tente novamente.</p>}
            <div className="flex justify-end border-t border-[var(--color-border)] pt-5"><Button type="submit" disabled={!rating || body.trim().length < 10} isLoading={mutation.isPending}>Enviar avaliação</Button></div>
          </form>
        )}
      </div>
    </div>
...
export default function ProductReviews({ productId, productTitle }: { productId: string; productTitle: string }) {
  ...
  const { data, isLoading, isError, refetch } = useProductReviews(productId, { sort, rating, verified, page })
  ...
  return (
    <section aria-labelledby="reviews-title" className="mt-14 border-t border-[var(--color-border)] bg-gradient-to-b from-[var(--color-surface-soft)]/45 to-transparent px-4 pb-6 pt-10 sm:px-6 lg:px-8 lg:pt-14">
      <div className="mx-auto flex max-w-7xl flex-col gap-7">
        ...
        {isLoading && !data ? <div className="space-y-4" aria-busy="true"><div className="h-64 animate-pulse rounded-2xl bg-white/80" /><div className="h-28 animate-pulse rounded-2xl bg-white/80" /></div> : isError ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-700" role="alert"><p>Não foi possível carregar as avaliações.</p><Button variant="outline" size="sm" className="mt-4" onClick={() => { void refetch() }}>Tentar novamente</Button></div> : total === 0 ? (
          <div className="rounded-2xl border border-dashed border-[var(--color-border)] bg-white p-6 shadow-[0_10px_26px_rgba(8,59,102,0.05)] sm:p-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--color-surface-soft)] text-2xl text-[var(--color-primary)]" aria-hidden="true">{String.fromCharCode(9733)}</div><div><h3 className="text-lg font-bold text-[var(--color-navy)]">Este produto ainda não possui avaliações.</h3><p className="mt-1 text-sm leading-6 text-[var(--color-text-muted)]">As avaliações de clientes aparecerão aqui assim que forem aprovadas.</p></div></div>
            {canWrite ? <div className="mt-6 flex flex-col gap-3 border-t border-[var(--color-border)] pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-medium text-[var(--color-navy)]">Você comprou este produto? Conte sua experiência.</p><Button onClick={openModal}>Escrever a primeira avaliação</Button></div> : <p className="mt-6 border-t border-[var(--color-border)] pt-5 text-sm text-[var(--color-text-muted)]">Você poderá avaliar este produto após uma compra elegível.</p>}
          </div>
        ) : (
          <>
      ...
      {modalOpen && <ReviewModal productId={productId} productTitle={productTitle} onClose={() => setModalOpen(false)} />}
    </section>
  )
}
