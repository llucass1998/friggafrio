      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-7">
        ...
        {isLoading && !data ? <div className="space-y-4" aria-busy="true"><div className="h-64 animate-pulse rounded-2xl bg-white/80" /><div className="h-28 animate-pulse rounded-2xl bg-white/80" /></div> : isError ? <p className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-700" role="alert">Não foi possível carregar as avaliações.</p> : total === 0 ? (
          <div className="rounded-2xl border border-dashed border-[var(--color-border)] bg-white p-6 shadow-[0_10px_26px_rgba(8,59,102,0.05)] sm:p-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--color-surface-soft)] text-2xl text-[var(--color-primary)]" aria-hidden="true">{String.fromCharCode(9733)}</div><div><h3 className="text-lg font-bold text-[var(--color-navy)]">Este produto ainda não recebeu avaliações.</h3><p className="mt-1 text-sm leading-6 text-[var(--color-text-muted)]">As avaliações de clientes aparecerão aqui assim que forem aprovadas.</p></div></div>
            {canWrite || canEdit ? <div className="mt-6 flex flex-col gap-3 border-t border-[var(--color-border)] pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm font-medium text-[var(--color-navy)]">{canEdit ? "Sua avaliação pode ser atualizada." : "Você comprou este produto? Conte sua experiência."}</p><Button onClick={openModal}>{canEdit ? "Editar avaliação" : "Escrever a primeira avaliação"}</Button></div> : <p className="mt-6 border-t border-[var(--color-border)] pt-5 text-sm text-[var(--color-text-muted)]">Você poderá avaliar este produto após uma compra elegível.</p>}
          </div>
        ) : (
          <>
