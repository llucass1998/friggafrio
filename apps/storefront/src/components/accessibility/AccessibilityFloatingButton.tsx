...
export function AccessibilityFloatingButton() {
  ...
  return (
    <button
      ...
      onClick={togglePanel}
      className={`fixed bottom-24 left-4 z-[9980] flex h-12 w-12 items-center justify-center rounded-full shadow-lg transition-[background-color,box-shadow,transform] duration-[var(--motion-duration-interaction)] hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--color-accent)] focus-visible:ring-offset-2 sm:left-6 ${preferences.panelEnabled ? "bg-[var(--color-navy)] text-white" : "bg-[var(--color-primary)] text-white"}`}
