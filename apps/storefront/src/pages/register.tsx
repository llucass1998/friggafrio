...
export default function RegisterPage() {
  ...
  const onSubmitPerson = async (data: PersonRegistrationForm) => {
    ...
    try {
      ...
      // Uses standard auth flow for B2C Customers
      await sdk.auth.register("customer", "emailpass", {
        email: data.email,
        password: data.password,
      })

      // Update customer details post auth/register inside Medusa
  ...
  const handleGoogleLogin = () => {
    // Redireciona para o fluxo Medusa OAuth se já configurado
    window.location.href = `${import.meta.env.VITE_MEDUSA_BACKEND_URL}/auth/customer/google`
  }
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[var(--color-border)] p-6 md:p-8 lg:p-10 w-full">
                ...
                <div className="pt-4 space-y-4">
                  <label className="flex items-start gap-3 cursor-pointer group">
                    ...
                    <span className="text-sm text-[var(--color-text)]">
                      Li e aceito os <Link to="/terms" className="text-[var(--color-primary)] hover:underline font-medium">Termos de Uso</Link> e a <Link to="/privacy" className="text-[var(--color-primary)] hover:underline font-medium">Política de Privacidade</Link>. <span className="text-red-500">*</span>
                    </span>
                  </label>
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full py-3.5 px-4 bg-white border border-[var(--color-border)] text-[var(--color-navy)] font-semibold rounded-[var(--radius-button)] hover:bg-[var(--color-surface-soft)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2 transition-colors flex items-center justify-center gap-3 disabled:opacity-70"
            >
