...
type State = "loading" | "success" | "already_confirmed" | "invalid" | "error"
...
export default function NewsletterActionPage({ action }: { action: Action }) {
  ...
  useEffect(() => {
    ...
    void request(token)
      .then((response) => setState(response.status === "confirmed" || response.status === "unsubscribed" ? "success" : response.status === "already_confirmed" ? "already_confirmed" : "invalid"))
      .catch(() => setState("error"))
  }, [action])

  const copy = action === "confirm"
    ? {
        title: "Confirmacao da newsletter",
        success: "Cadastro confirmado! Agora voce recebera as novidades da FriggaFrio.",
        alreadyConfirmed: "Este cadastro ja estava confirmado. Voce continuara recebendo as novidades da FriggaFrio.",
        invalid: "Este link e invalido ou expirou. Solicite um novo cadastro pelo site.",
      }
    : {
        title: "Descadastro da newsletter",
        success: "Seu cadastro foi cancelado. Voce nao recebera novas comunicacoes.",
        alreadyConfirmed: "Seu cadastro ja estava cancelado.",
        invalid: "Este link de descadastro e invalido.",
      }
  ...
  const message = state === "loading"
    ...
    : state === "success"
      ...
      : state === "already_confirmed"
        ...
        : state === "invalid"
          ? copy.invalid
          : "Nao foi possivel processar seu pedido agora. Tente novamente mais tarde."
  ...
  return (
    <main className="flex min-h-[calc(100vh-8rem)] items-center justify-center bg-[#f3f9fd] px-4 py-12 sm:px-6">
      <section className="w-full max-w-xl rounded-[22px] border border-[#d9e8f2] bg-white p-7 text-center shadow-[0_16px_42px_rgba(8,59,102,0.10)] sm:p-10" aria-live="polite">
        ...
        <p className="mt-4 text-sm leading-7 text-[var(--color-text-muted)]">{message}</p>
