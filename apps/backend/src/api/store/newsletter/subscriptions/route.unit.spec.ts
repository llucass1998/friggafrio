...
describe("newsletter subscriptions", () => {
  it("creates a pending, consented subscription with hashes only and gives a generic response", async () => {
  ...
  it("accepts honeypot submissions without persisting any address", async () => {
    const res = response()
