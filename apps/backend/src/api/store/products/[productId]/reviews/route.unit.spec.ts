...
const fixture = ({ eligible = true, existing = false } = {}) => {
  ...
  const query = {
    graph: jest.fn(async ({ entity, filters }: { entity: string; filters?: { id?: string } }) => {
      if (entity === "product") return { data: filters?.id === "prod_1" ? [{ id: "prod_1" }] : [] }
...
describe("product review routes", () => {
  ...
  it("rejects an unknown product instead of treating it as an empty review collection", async () => {
    const { request } = fixture()
    const req = request()
    req.params.productId = "prod_missing"
    await expect(GET(req as never, response() as never)).rejects.toMatchObject({ type: MedusaError.Types.NOT_FOUND })
  })
