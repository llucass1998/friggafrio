...
const reviewServiceFor = (req: MedusaRequest): ReviewService =>
...
const customerIdFrom = (req: AuthenticatedMedusaRequest): string => {
  const customerId = req.auth_context?.actor_id
  if (!customerId) throw new MedusaError(MedusaError.Types.UNAUTHORIZED, "Authentication required")
  return customerId
}

const productExists = async (req: MedusaRequest, productId: string): Promise<boolean> => {
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  if (!(await productExists(req, productId))) {
...
export const reviewEligibility = async (req: AuthenticatedMedusaRequest, res: MedusaResponse) => {
  ...
  res.json({ eligible: Boolean(order), already_reviewed: existing.length > 0 })
}
