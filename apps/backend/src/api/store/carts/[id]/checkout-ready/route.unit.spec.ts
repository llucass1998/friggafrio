...
const makeCart = () => ({
  ...
  shipping_address: {
    ...
    city: "Sao Paulo", postal_code: "01310-100", country_code: "br",
  },
...
describe("legacy checkout-ready boundary", () => {
  it("does not reserve inventory and requires a signed prepare marker", async () => {
    ...
    const res = response()
    await POST({ params: { id: cart.id }, scope: { resolve: (key: unknown) => key === ContainerRegistrationKeys.QUERY ? query : undefined } } as never, res as never)
