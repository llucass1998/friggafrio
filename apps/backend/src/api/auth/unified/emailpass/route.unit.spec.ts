...
const makeRequest = (body: Record<string, unknown>, auth: Record<string, unknown>) => {
...
describe("unified emailpass login", () => {
  it("rejects actor hints from the browser", async () => {
    ...
    const request = makeRequest({ email: "customer@example.com", password: "secret", actor: "user" }, auth)
    await expect(POST(request, makeResponse() as never)).rejects.toMatchObject({ type: "unauthorized" })
  ...
  it("creates a customer session without returning actor information", async () => {
    const identity = {
      ...
      provider_identities: [{ provider: "emailpass", entity_id: "customer@example.com", user_metadata: {} }],
    }
    const auth = {
      listProviderIdentities: jest.fn().mockResolvedValue([{ provider: "emailpass", entity_id: "customer@example.com", auth_identity: identity }]),
      authenticate: jest.fn().mockResolvedValue({ success: true, authIdentity: identity }),
    }
    const request = makeRequest({ email: " Customer@Example.com ", password: "secret" }, auth) as any
    const response = makeResponse()

    await POST(request, response as never)

    expect(auth.authenticate).toHaveBeenCalledWith("emailpass", expect.objectContaining({ actor_type: "customer" }))
    expect(request.session.auth_context).toMatchObject({ actor_id: "cus_1", actor_type: "customer" })
  ...
  it("resolves a Medusa user as an admin session server-side", async () => {
    const identity = {
      ...
      provider_identities: [{ provider: "emailpass", entity_id: "admin@example.com", user_metadata: {} }],
    }
    const auth = {
      listProviderIdentities: jest.fn().mockResolvedValue([{ provider: "emailpass", entity_id: "admin@example.com", auth_identity: identity }]),
      authenticate: jest.fn().mockResolvedValue({ success: true, authIdentity: identity }),
    }
    const request = makeRequest({ email: "admin@example.com", password: "secret" }, auth) as any
    const response = makeResponse()

    await POST(request, response as never)

    expect(auth.authenticate).toHaveBeenCalledWith("emailpass", expect.objectContaining({ actor_type: "user" }))
