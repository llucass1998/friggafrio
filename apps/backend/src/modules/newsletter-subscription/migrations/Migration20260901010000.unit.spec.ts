...
describe("newsletter pending status migration", () => {
  it("replaces only the legacy status constraint and accepts pending confirmation", () => {
    const source = readFileSync(join(__dirname, "Migration20260901010000.ts"), "utf8")
    ...
    expect(source).toContain("''pending'', ''active'', ''unsubscribed'', ''bounced'', ''complained''")
    ...
    expect(source).not.toMatch(/delete|truncate|drop table|if exists \"newsletter_subscription\"/i)
  })
})
